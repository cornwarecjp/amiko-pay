...
class Test(unittest.TestCase):
	...
	def test_payeeSide(self):
		'Test behavior when route times out on the payee side'

		log.log('\n\n\n\nSCENARIO TEST: largenetwork_routeTimeout.py test_payeeSide\n')

		settings = largenetwork_setup.makeNodes()

		#Let node 5 ignore MakeRoute:
		with open(settings[5].stateFile, 'rb') as f:
			stateData = f.read()
		stateData = stateData.replace('NodeState', 'NodeState_IgnoreMakeRoute')
		with open(settings[5].stateFile, 'wb') as f:
			f.write(stateData)
	...
	#TODO: test whether payee, intermediate nodes also time-out if payer doesn't!
